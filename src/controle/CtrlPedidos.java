...
public class CtrlPedidos {
	...
	public String inserirPedidoEx(Cliente cliente, Cozinheiro cozinheiro,
		...
		vetorP[i] = new Externo(cliente, cozinheiro, entregador, atendente);

		return "Pedido Inserido";
	}

	public String inserirPedidoIn(Cozinheiro cozinheiro, Mesa mesa,
			Garçom garçom) {
		int i;
		for (i = 0; i < vetorP.length; i++) {
			if (vetorP[i] == null)
	...
	public String InserirItem(Itens item, int codigo) {
		for (int i = 0; i < vetorP.length; i++) {
			if (vetorP[i] != null) {
				if (vetorP[i].getCodigo() == codigo)
					return vetorP[i].InserirItem(item);
			}
		}
	...
	public String getItens(int codigo) {
		for (int i = 0; i < vetorP.length; i++) {
			if (vetorP[i] != null) {
				if (vetorP[i].getCodigo() == codigo) {
					return vetorP[i].getItens();
				}
			}
		}
		return "Pedido não encontrado";
